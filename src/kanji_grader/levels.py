...
class Levels:
    ...
    def __init__(self):
        ...
        json_filepath = os.path.join(script_path, "resources", "kanji-jouyou.json")
    ...
    def get_jlpt_level(self, kanji: str) -> Union[int, None]:
        ...
        if kanji is None or len(kanji) < 1:
            raise ValueError(
                "get_jlpt_level accepts a single character, but you gave it nothing."
            )
        elif len(kanji) > 1:
            raise ValueError(
                f"get_jlpt_level accepts a single character, but your text was {len(kanji)} characters."
            )
    ...
    def get_grade_level(self, kanji: str) -> Union[int, None]:
        ...
        if kanji is None or len(kanji) < 1:
            raise ValueError(
                "get_grade_level accepts a single character, but you gave it nothing."
            )
        elif len(kanji) > 1:
            raise ValueError(
                f"get_grade_level accepts a single character, but your text was {len(kanji)} characters."
            )
